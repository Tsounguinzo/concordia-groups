<script lang="ts">
    import type {INPUT_CONFIG} from "$lib/UI-LIBRARY/types"
    ...
    } from "$lib/UI-LIBRARY/functions"
    import {FORM, HelperText, Label} from "$lib/UI-LIBRARY"
    ...
    export let typingEffect: boolean = true
    ...
    let typingStatus = (isTyping: boolean) => isUserTyping = isTyping;
</script>
...
<div class={getInputContainerClass(C)}>
    ...
    <div class={getInputBoxClass(C, $$restProps)}>
        ...
        <input {...$$restProps} {id} {name}
               class={getFormControlStyle(C, $$restProps, $$props?.class, "input", $$slots)}
               bind:value
               use:typing={typingStatus}
               use:setType
               on:blur
               on:change
               on:click
