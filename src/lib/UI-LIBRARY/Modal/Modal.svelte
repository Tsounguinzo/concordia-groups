<script lang="ts">
    ...
    import {generateToken, getAnimate, getRounded} from "$lib/UI-LIBRARY/functions";
    import type {ANIMATE_SPEED, ROUNDED} from "$lib/UI-LIBRARY/types";
    import {modalIdStore} from "$lib/UI-LIBRARY/stores";
    import Close from "$lib/UI-LIBRARY/Utils/Close.svelte";
...
{#if $$slots.body}
    <div id="{id}Modal" class={twMerge(modalBodyCls(), modalOuterClass)} role="dialog"
         class:open={$modalIdStore.includes(id)} class:animate={animate}>
        {#if backdrop}
            <!-- svelte-ignore a11y-click-events-have-key-events -->
        ...
        <div class={twMerge(modalContentCls(), (size !== "full" ? getRounded(rounded) : ""), modalInnerClass)}
            ...
            {#if $$slots.header}
                <div id="{id}Heading"
                     class={twMerge("modal-header flex justify-between w-full gap-8 items-start border-b border-black/10 dark:border-tertiary pb-4 mb-8", modalHeaderClass)}>
                    <slot name="header"></slot>
                    {#if closeBtn !== false}
                        <button class="text-default flex-grow-0 opacity-25 hover:opacity-75 transition-opacity"
                                on:click={()=>toggle()}>
                            <Close/>
                        </button>
                    {/if}
            ...
            {:else if closeBtn !== false}
                <button class="text-default flex-grow-0 opacity-25 hover:opacity-75 transition-opacity absolute top-2 right-2"
                        on:click={()=>toggle()}>
                    <Close/>
                </button>
            {/if}
